...
class InvolvementsGenerator < Generators::EventBase
  ...
  def add_involvement_entry
    ...
    involvement_entry["users"] = options[:users] if options[:users]
    involvement_entry["organisations"] = options[:organisations] if options[:organisations]
    yerba_document << involvement_entry
    yerba_document.save!
  end
end
