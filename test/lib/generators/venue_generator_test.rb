...
class VenueGeneratorTest < Rails::Generators::TestCase
  ...
  test "venue.yml passes schema validation" do
    ...
    validate_venue_schema File.join(destination_root, "data/rubyconf/2024/venue.yml")
  end

  test "venue with all flags passes schema validation" do
    run_generator ["--event-series", "rubyconf", "--event", "2024", "--hotel", "--nearby", "--locations", "--rooms", "--spaces", "--accessibility"]

    validate_venue_schema File.join(destination_root, "data/rubyconf/2024/venue.yml")
  end

  test "venue with all flags off passes schema validation" do
    run_generator ["--event-series", "rubyconf", "--event", "2024", "--no-hotel", "--no-nearby", "--no-locations", "--no-rooms", "--no-spaces", "--no-accessibility"]
