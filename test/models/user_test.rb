...
class UserTest < ActiveSupport::TestCase
  ...
  test "github_account_empty? returns true when all counts are zero" do
    user = User.create!(
    ...
    assert user.github_account_empty?
  end

  test "github_account_empty? returns false when has repos" do
    user = User.create!(
    ...
    assert_not user.github_account_empty?
  end

  test "github_account_empty? returns false when has followers" do
    user = User.create!(
  ...
  test "github_account_empty? returns false when metadata blank" do
    user = User.create!(name: "No Metadata User 2", github_metadata: {})

    assert_not user.github_account_empty?
  end

  test "bio_contains_url? returns true when bio has http URL" do
    user = User.create!(name: "URL Bio User", bio: "Check out http://example.com")

    assert user.bio_contains_url?
  end

  test "bio_contains_url? returns true when bio has https URL" do
    user = User.create!(name: "HTTPS Bio User", bio: "Visit https://example.com for more")

    assert user.bio_contains_url?
  end

  test "bio_contains_url? returns false for normal bio" do
    user = User.create!(name: "Normal Bio User", bio: "Ruby developer from Berlin")

    assert_not user.bio_contains_url?
  end

  test "bio_contains_url? returns false for blank bio" do
