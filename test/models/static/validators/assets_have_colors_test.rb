...
class Static::Validators::AssetsHaveColorsTest < ActiveSupport::TestCase
  ...
  test "flags a real event asset without colors" do
    series_slug, event_slug = Dir.glob(Rails.root.join("app/assets/images/events/*/*/featured.webp")).first.split("/")[-3..-2]

    event = {"name" => "TestConf", "kind" => "conference"}.to_yaml
    with_temp_event(event, series_slug: series_slug, event_slug: event_slug) do |file_path|
  ...
  test "does not flag when colors are defined" do
    series_slug, event_slug = Dir.glob(Rails.root.join("app/assets/images/events/*/*/featured.webp")).first.split("/")[-3..-2]
    has_banner = File.exist?(Rails.root.join("app/assets/images/events", series_slug, event_slug, "banner.webp")) ||
      File.exist?(Rails.root.join("app/assets/images/events", series_slug, "default", "banner.webp"))

    event = {"name" => "TestConf", "featured_background" => "#FFFFFF", "featured_color" => "#000000"}
    event["banner_background"] = "#FFFFFF" if has_banner

    with_temp_event(event.to_yaml, series_slug: series_slug, event_slug: event_slug) do |file_path|
  ...
  def with_temp_event(event_yaml, series_slug: "testconf", event_slug: "testconf-2025")
    dir = Dir.mktmpdir
