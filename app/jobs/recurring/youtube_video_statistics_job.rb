class Recurring::YouTubeVideoStatisticsJob < ApplicationJob
  ...
  def perform
    step :iterate_videos do |step|
      puts "cursor at start of step: #{step.cursor}"
      Talk.youtube.in_batches(of: BATCH_SIZE, start: step.cursor) do |talks|
        puts "Processing batch starting at #{talks.first.id}"
        puts "batch size: #{BATCH_SIZE}, talks batch size: #{talks.size}"
        puts "Cursor = #{step.cursor}"
